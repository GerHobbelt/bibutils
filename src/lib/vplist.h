...
// pass reference to `const void *` data pointer to free and NULL (for safety):
typedef void (*vplist_ptrfree)(const void**);
...
int    vplist_removevp      ( vplist *vpl, const void *v );
int    vplist_removevpfn    ( vplist *vpl, const void *v, vplist_ptrfree vpf );
...
vplist_index vplist_find( vplist *vpl, const void *v );
